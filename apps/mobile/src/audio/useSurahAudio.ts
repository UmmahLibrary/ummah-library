...
export interface SurahAudio {
  ...
  /** True if the most recent `downloadSurahs` call failed partway through. */
...
export function useSurahAudio(reciter: ReciterPlugin): SurahAudio {
  ...
  // Reflect which of this reciter's surahs are already downloaded. Runs on
  // every mount of every surah/juzʾ reader screen, so a storage read failure
  // here (corrupted state, a revoked permission, an unsupported platform)
  // must not become an unhandled rejection on every navigation — fall back to
  // "nothing downloaded yet" instead, same as `savedSurahs()`'s own
  // no-directory-yet case.
  const refreshSaved = useCallback(() => {
    void mobileAudioStore
      .savedSurahs()
      .catch(() => [])
      .then((saved) => {
        setSavedSurahs(
    ...
    (surahs: number[]) => {
      ...
      void (async () => {
        try {
          ...
          // audio-store's save() already cleans up any partial file it left
          // behind — this just stops the failure from becoming an unhandled
          // promise rejection and lets the button tell the user to retry
          // instead of silently going back to "Download for offline listening".
  ...
  const startSession = useCallback(
    (queue: VerseKey[], startKey: VerseKey, repeat: number | null) => {
      ...
      void (async () => {
        ...
        // Everything below can throw before ever reaching the per-āyah
        // `new Promise` that owns cleanup (a storage read failing in
        // `mobileAudioStore.localUrl`, a bad/unreachable audio source
        // rejecting `ensurePlayer`, …) — without this, that throw becomes an
        // unhandled rejection AND leaves `buffering`/`playingKey` stuck
        // exactly as set at the top of `startSession`, so the UI shows
        // "Loading…" forever with no recovery but restarting the app.
        try {
    ...
    (verse: VerseKey, wordIndex: number) => {
      ...
      void (async () => {
        try {
          ...
          // A storage read (mobileAudioStore.localUrl) or player init failing
          // here would otherwise be an unhandled rejection — this path never
          // sets `buffering`, so there's no stuck spinner to clear, just the
          // word highlight/playing-key it may have already set.
          if (tokenRef.current === token) {
            setActiveWord(-1);
            setPlayingKey(null);
          }

 ...
 * The secret is wrapped with a non-extractable AES-256-GCM key kept in IndexedDB,
 * so the value that actually lands in `localStorage` is ciphertext, not the
 * secret itself, and the unwrapping key can never be exported or copied out —
 * only this origin's own code, calling this exact key, can turn it back.
 *
 ...
 * `localStorage` from devtools, a profile-sync/backup tool, or disk access
 * without code execution no longer yields the secret directly.
 */
 ...
 * or WebCrypto is unavailable (private-mode restrictions, an old browser) —
 * best-effort hardening, never a reason to break sync setup.
 */

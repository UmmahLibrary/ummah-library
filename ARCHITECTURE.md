...
| **`adapters`**    | Concrete adapters for **external** concerns behind core ports: `HttpTafsirRepository`, `HttpHadithRepository`, `InMemoryHifzRepository`, `SqliteHifzRepository` (subpath), `AdhanPrayerTimes`. | `core`                     |
...
- `adhkar.ts` — adhkar occasion catalogue + pure session-counter helpers (`filterByOccasion`, `nextTally`, `sessionProgress`); the `Dhikr` text is bundled content ([ADR 0016](docs/adr/0016-adhkar.md)).

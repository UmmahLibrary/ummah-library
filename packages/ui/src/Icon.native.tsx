...
interface IconProps {
  ...
  sw?: number;
  color?: string;
  /** Ignored on native — use color prop instead. */
  style?: CSSProperties;
}

export function Icon({ name, size = 20, sw = 1.7, color = "currentColor" }: IconProps) {
  const stroke = color === "currentColor" ? "#ffffff" : color;

  if (name === "play") {
    return (
      <Svg width={size} height={size} viewBox="0 0 24 24">
        <Path d="M7 4.5v15l13-7.5-13-7.5Z" fill={stroke} />
      </Svg>
    );
  }
  if (name === "pause") {
